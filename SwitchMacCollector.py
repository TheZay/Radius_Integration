...
def setup_logging() -> None:
    ...
    :return: None
    """
    log_file_name = f'nmc_{datetime.now().strftime("%Y%m%d_%H%M%S")}.log'
...
def create_member_element(members: ET.SubElement, mac_address: str) -> None:
    """
...
def save_formatted_xml(xml_string: str, base_file_name: str) -> None:
    ...
    :return: None
    """
    output_file_name = f'{base_file_name}.xml'
...
def safe_exit(script_start_timer: float = None, device_counter: int = None) -> None:
    """
...
def connect_to_device(device: dict, hostname: str) -> ConnectHandler:
    """
...
def process_devices(devices: dict, credentials: dict) -> set[str]:
    ...
    for device in devices['hosts']:
        ...
        try:
            ...
            disconnect_from_device(ssh, device, hostname)

    return mac_addresses


def process_device_data(ssh: ConnectHandler, mac_addresses: set[str]) -> None:
    """
...
def get_vlan_output(ssh: ConnectHandler) -> list[dict]:
    """
...
def get_mac_address_table(ssh: ConnectHandler, vlan_id: int) -> list[dict]:
    """
...
def send_command(ssh: ConnectHandler, command: str) -> list[dict]:
    """
...
def disconnect_from_device(ssh: ConnectHandler, device: dict, hostname: str = None) -> None:
    """
